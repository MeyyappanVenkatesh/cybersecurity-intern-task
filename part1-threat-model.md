...
\*\*Threat\*\*: Attacker manipulates object identifiers (`customer\_id`, `company\_id`) in API requests to access/modify data belonging to other tenants without any prior account compromise.
...
\*\*Threat\*\*: Application generates presigned URLs or public S3 paths without tenant-scoped keys (e.g., `s3://clienthub-files/customer\_789.pdf` vs `s3://clienthub-files/company\_1234/customer\_789.pdf`). Attacker enumerates object keys.
